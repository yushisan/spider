var request = require('request');
var parser = require('./parser');
var fs = require('fs');
var path = require('path');
var $ = require('cheerio');


/**
 * Spider
 */
var Spider = function(options, callback, handlerMap) {
    if (typeof callback === 'object') {
        handlerMap = callback;
        callback = function() {};
    }
    return request.apply(this, [handlerOpts(options), getHandler(callback, handlerMap)]);
};

Spider.get = function(url, callback, handlerMap) {
    if (typeof callback === 'object') {
        handlerMap = callback;
        callback = function() {};
    }
    return request.get(handlerOpts(url), getHandler(callback, handlerMap));
};
Spider.post = function(url, data, callback, handlerMap) {
    if (typeof callback === 'object') {
        handlerMap = callback;
        callback = function() {};
    }

    return request.post(handlerOpts(url), getHandler(callback, handlerMap));
};

//保存
Spider.save = function(url, output, callback) {
    if (typeof output === 'function') {
        callback = output;
        output = path.basename(url);
    } else {
        output = output || path.basename(url);
    }
    url = handlerOpts(url);
    return request(url)
        .on('error', callback)
        .on('end', callback)
        .pipe(fs.createWriteStream(output));
};

Spider.request = request;

module.exports = Spider;

function getHandler(callback, handlerMap) {
    return function(error, response, body) {
        if (!error) {
            var contentType = response.headers['content-type'];
            //处理json
            if (contentType.indexOf('application/json') !== -1) {
                return callback(error, JSON.parse(body));
            } else if (contentType.indexOf('text/html') !== -1 || contentType.indexOf('/xml') !== -1) {
                return callback(error, parser(body, handlerMap), response);
            }
            callback(error, body, response);
        } else {
            callback(error, body, response);
        }
    };
}

/**
 * 统一处理参数，处理cookie机制
 * @param  {[type]} url [description]
 * @return {[type]}     [description]
 */
function handlerOpts(url) {
    var options = typeof url === 'string' ? {
        url: url
    } : url;
    if (options.cookie) {
        var j = options.jar || request.jar();
        var cookie = request.cookie(options.cookie);
        j.setCookie(cookie, options.url, function(err, cookie) {
            if (err) {
                console.log('set cookie error:', err);
            }
        });
        options.jar = j;
    }
    return options;
}
